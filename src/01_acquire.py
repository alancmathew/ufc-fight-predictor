...
class EventsProcessor:
        ...
        for file in page_files:
            ...
            with open(filepath, "r") as f:
                ...
                row_elems = lfilter(lambda r: \
                                    len(r.find_all("a", class_="b-link b-link_style_black")) != 0, \
                                                row_elems)
    ...
    def get_eventlist_dfs(self):
        self.completed_events_df = \
                self.get_eventlist_df(dir_dict["completed_eventlist_html"], "completed_events.csv")
        
        self.upcoming_events_df = \
                self.get_eventlist_df(dir_dict["upcoming_eventlist_html"], "upcoming_event.csv")
...
def start_processor():
    ...
    events_processor = EventsProcessor(events_conf)

    events_processor.start()

    print("\nCompleted Events Processing\n")
    
    fights_processor = FightsProcessor()

    fights_processor.start()
    
    print("\nCompleted Fights Processing\n")

    fighters_processor = FightersProcessor()

    fighters_processor.start()
    
    print("\nCompleted Fighters Processing\n")
